...
#
repo_information() {
    # Only show git info if we're in a git repo
    command git rev-parse --is-inside-work-tree &>/dev/null || return
    local repo_name=$(basename $(git rev-parse --show-toplevel) 2>/dev/null)
    local branch=$(git rev-parse --abbrev-ref HEAD 2>/dev/null)
    local dirty=$(git_dirty)
    # echo "%F{8}${repo_name}%f:%F{8}${branch}${dirty}%f"
    echo ":%F{8}${branch}${dirty}%f"
}
...
# Define prompts
#
PROMPT='%F{yellow}${(C)${PWD:t}}%f$(repo_information)[$(python_version)]# '
RPROMPT=""
